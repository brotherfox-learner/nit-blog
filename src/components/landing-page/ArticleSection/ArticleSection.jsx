    ...
    <section className="flex flex-col gap-[16px] py-[16px] min-[1280px]:px-[120px] min-[1280px]:h-[144px] group/section">
      <h2 className="font-semibold text-[24px] leading-[32px] text-[#26231E] pl-[16px] min-[1280px]:pl-0 transition-all duration-500  group-hover/section:translate-x-1">
        ...
        <span className="inline-flex items-center ml-2 opacity-0 group-hover/section:opacity-100 transition-all duration-700 group-hover/section:translate-x-1 relative w-8 h-8">
          ...
          <svg className="w-5 h-5 text-[#FFD700] animate-spin-slow absolute top-0 left-0 drop-shadow-[0_0_10px_rgba(255,215,0,0.9)]" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z" />
          </svg>
          
          {/* Second Sparkle - Black with Gold Glow */}
          <svg className="w-4 h-6 text-[#fff130] animate-ping absolute top-0 left-8 delay-100 drop-shadow-[0_0_8px_rgba(255,215,0,1)]" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2l2.4 7.2h7.6l-6 4.8 2.4 7.2-6-4.8-6 4.8 2.4-7.2-6-4.8h7.6z" />
          </svg>
          
          {/* Third Sparkle - Bright Gold Dot */}
          <svg className="w-2 h-2 text-[#636363] animate-pulse absolute bottom-0.5 left-2 drop-shadow-[0_0_6px_rgba(255,215,0,1)]" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="hidden min-[1280px]:flex min-[1280px]:items-center min-[1280px]:gap-[8px] relative z-10">
          {categories.map((category, index) => (
            <Button
              key={category}
              variant="ghost"
              className="min-w-[70px] relative overflow-hidden font-medium text-[16px] leading-[24px] text-[#75716B] backdrop-blur-sm hover:bg-[#1a1a1a] hover:text-white hover:border-[#1a1a1a] hover:shadow-xl rounded-[10px] h-[48px] px-4 transition-all duration-300 ease-out hover:scale-105 active:scale-95 hover:-translate-y-1 group/button"
              style={{
                animationDelay: `${index * 100}ms`,
                transition: 'all 0.3s ease-out'
              }}
            >
              {/* Button Background Gradient on Hover - Black to Gray */}
              <span className="absolute inset-0 bg-linear-to-r from-[#000000] via-[#242424] to-[#363636] opacity-0 group-hover/button:opacity-100 transition-opacity duration-300"></span>
              
              {/* Sparkle Effect - White/Gold */}
              <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-white rounded-full opacity-0 group-hover/button:opacity-100 group-hover/button:animate-ping shadow-[0_0_6px_rgba(255,255,255,0.8)]"></span>
              
              <span className="relative z-10 transition-colors duration-300">{category}</span>
